# Cryptosuite

TypeScript implementation of [Data Integrity BIP340 Cryptosuite v0.1 specification](https://dcdpr.github.io/data-integrity-schnorr-secp256k1/).
Designed for use by [@did-btcr2/method](../../_media/README.md).

## Classes

- [Cryptosuite](classes/Cryptosuite.md)
- [DataIntegrityProof](classes/DataIntegrityProof.md)
- [SchnorrMultikey](classes/SchnorrMultikey.md)

## Interfaces

- [CreateProofParams](interfaces/CreateProofParams.md)
- [CryptosuiteParams](interfaces/CryptosuiteParams.md)
- [DidParams](interfaces/DidParams.md)
- [FromPublicKey](interfaces/FromPublicKey.md)
- [FromPublicKeyMultibaseParams](interfaces/FromPublicKeyMultibaseParams.md)
- [FromSecretKey](interfaces/FromSecretKey.md)
- [GenerateHashParams](interfaces/GenerateHashParams.md)
- [ICryptosuite](interfaces/ICryptosuite.md)
- [IDataIntegrityProof](interfaces/IDataIntegrityProof.md)
- [Multikey](interfaces/Multikey.md)
- [ProofSerializationParams](interfaces/ProofSerializationParams.md)
- [ProofVerificationParams](interfaces/ProofVerificationParams.md)
- [TransformDocumentParams](interfaces/TransformDocumentParams.md)
- [VerificationResult](interfaces/VerificationResult.md)
- [VerifyProofParams](interfaces/VerifyProofParams.md)

## Type Aliases

- [AddProofParams](type-aliases/AddProofParams.md)
- [MultikeyObject](type-aliases/MultikeyObject.md)
