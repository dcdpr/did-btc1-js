import { Btcr2Error, DidUpdateInvocation, Proof, PROOF_GENERATION_ERROR, PROOF_PARSING_ERROR } from '@did-btcr2/common';
...
export class DataIntegrityProof implements IDataIntegrityProof {
  ...
  public async addProof({ document, options }: AddProofParams): Promise<DidUpdateInvocation> {
    ...
    if (!type || !verificationMethod || !proofPurpose) {
      throw new Btcr2Error('Proof missing "type", "verificationMethod" and/or "proofPurpose"', PROOF_GENERATION_ERROR, proof);
    }
    ...
    if (options.domain && options.domain !== domain) {
      throw new Btcr2Error('Domain mismatch between options and domain passed', PROOF_GENERATION_ERROR, proof);
    }
    ...
    if (options.challenge && options.challenge !== challenge) {
      throw new Btcr2Error('Challenge mismatch options and challenge passed', PROOF_GENERATION_ERROR, proof);
    }
  ...
  public async verifyProof({
    ...
    if (typeof secure !== 'object' || typeof proof !== 'object') {
      throw new Btcr2Error('', PROOF_PARSING_ERROR);
    }
    ...
    if (!type || !verificationMethod || !proofPurpose) {
      throw new Btcr2Error('', 'PROOF_VERIFICATION_ERROR');
    }
